...
test("production source includes privacy, identity, and live-tracking safeguards",async()=>{const [platform,api,map,styles]=await Promise.all([readFile(new URL("../app/Platform.tsx",import.meta.url),"utf8"),readFile(new URL("../app/api/actions/route.ts",import.meta.url),"utf8"),readFile(new URL("../app/LiveHelpMap.tsx",import.meta.url),"utf8"),readFile(new URL("../app/fixes.css",import.meta.url),"utf8")]);assert.match(platform,/revealed only to the helper whose offer you accept/);assert.match(platform,/navigator\.geolocation\.watchPosition/);assert.match(api,/Only the person who requested help can accept an offer/);assert.match(api,/Only the accepted helper can share delivery progress/);assert.match(map,/openstreetmap\.org/);assert.match(styles,/@media\(max-width:760px\)/)});
