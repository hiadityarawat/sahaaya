...
test("sidebar brand remains contained without the distortion-prone sheen",async()=>{const styles=await read("app/fixes.css");assert.match(styles,/\.side-brand:after\{content:none!important;display:none!important\}/);assert.match(styles,/\.side-brand>\.brand-logo-mark\{[\s\S]*?box-shadow:none;[\s\S]*?animation:none!important/);assert.match(styles,/@keyframes sideBrandLetterFade/)});
